...
const OptionModalCard = ({ 
    ...
    return (
        <div className={`flex flex-col px-8 py-10 rounded-lg border border-[#dddddd] ${isAvailable ? '' : 'opacity-50'}`}>
            ...
            <div className='flex flex-row justify-between items-center'>
                ...
                <button className={`text-white text-sm font-medium ${isAvailable ? 'bg-moderateCyan hover:bg-darkCyan transition-colors ease-in duration-200' : 'bg-darkGray'} rounded-full px-8 py-3`} disabled={!isAvailable} onClick={toggleDonationFunction} >{isAvailable ? 'Select Reward' : 'Out of stock'}</button>
            </div>
        </div>
    )
}
